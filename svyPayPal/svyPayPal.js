...
function getHeadlessClient() {
	...
	if (!client) {
		...
		if (!uid || !security.checkPassword(uid, remoteUserPassword) || security.getUserGroups(uid).getMaxRowIndex() < 1)
			throw new scopes.svyExceptions.IllegalStateException('Failed to authenticate remote dispatch user "' + remoteUserName + '" for PayPal Remote Dispatch Client. ' + 'Ensure that the account exists and has correct credentials and is valid/belongs to a valid group.');
